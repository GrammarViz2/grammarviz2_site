...
weight: 101
pagekind: "reading"
summary: "Discusses SAX discretization parameters optimization approach implemented in GrammarViz 3.0."
labels:
  - algorithms
---
...
\frac{
 \sum\limits_{i=p}^{i \lt p+n}\sqrt{(C_{i}-PAA_{C_{i}})^{2}}
}{n}
$$
...
\frac{
 \sum\limits_{i=0}^{i \lt k}\sqrt{(C_{PAA^{k}}^{i}-CutCenter(C_{PAA^{k}}^{i}))^{2}
 }
}{k}
$$

where \(k\) is the PAA size and \(C_{PAA^{k}}\) is the PAA transform of \(C\). The value of the \(CutCenter(C_{PAA^{k}}^{i})\) is taken from the table of precomputed values similar to the SAX cut lines table. 
...
{{< fig src="sax-error-results.png" w="800" >}}
...
{{< fig src="sampler-screen.png" w="800" >}}
